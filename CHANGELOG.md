...
[Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and once a release
is tagged the project will use [Semantic Versioning](https://semver.org).
...
- **Cache correctness (P0):** `rote.clear()` now wipes the per-wrapper
  in-memory tier as well as SQLite + blobs. Previously the in-memory entry
  survived the clear and the next call returned a stale hit without re-running
  the body.
...
- **File dependencies:** `file_dep_hash()` no longer blocks indefinitely on
  FIFOs / non-regular files; cache-dir *sibling* paths (e.g. `.rote-input.txt`
  next to `.rote/`) are no longer falsely excluded from dependency tracking
  because of a naive `str.startswith` check.
- **Purity:** the static impure-callee set is recomputed when a function's
  ...
  invalidates previously-cached results that called it.

### Changed

- **Bounded memory:** five formerly-unbounded structures now have caps so
  long-running Jupyter kernels stay within memory:
  - `_PERF_BLACKLIST` — LRU, 4096 entries
  - `_Session.call_graph` — LRU, 8192 distinct callers
  - `PurityTracker.verdicts` — FIFO, 1024 entries
  - `Tracer.buffer` events drop `code` and `return_value` references after
    listener dispatch (kept only the metadata needed for spill)
  - `Store._pending_hits` auto-flushes at 1024 entries
- **Startup cost:** PEP 578 audit hooks are deferred until the first
  `@cache` / `auto()` use. `import rote` no longer pays per-event hook
  dispatch on every file open or socket call.
- **CI matrix:** added `windows-latest` to the existing Linux + macOS jobs.

### Removed

- Dead helpers: `PurityTracker.attach_args`, `pop_verdict_for`, `note_close`;
  `compute_arg_fingerprints`, `mutable_arg_names`; `_CONTENT_HASH_LIMIT`;
  `identity.module_source_hash`; `Config.verbose`,
...
`rote.configure(...)`, `rote.stats()`, `rote.graph()`, and the
